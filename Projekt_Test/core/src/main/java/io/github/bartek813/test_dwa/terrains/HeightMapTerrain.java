...
import io.github.bartek813.test_dwa.terrains.attributes.TerrainTextureAttribute;
...
public class HeightMapTerrain extends Terrain{
    ...
    public float getHeightAtWorldCoord(float worldX, float worldZ) {
        // Convert world coordinates to a position relative to terrain
        modelInstance.transform.getTranslation(c00);
        float terrainX = worldX - c00.x;
        float terrainZ = worldZ - c00.z;

        // float size between vertices
        float gridSquaresSize = size / ((float) width -1);

        // determine grid square
        int gridX = (int) Math.floor(terrainX / gridSquaresSize);
        int gridZ = (int) Math.floor(terrainZ / gridSquaresSize);

        // Validates grid square
        if (gridX >= width -1 || gridZ >= width -1 || gridX < 0 || gridZ < 0) {
            return 0;
        }

        // determine where on the grid square coordinates are
        float xCoord = (terrainX % gridSquaresSize) / gridSquaresSize;
        float zCoord = (terrainZ % gridSquaresSize) / gridSquaresSize;

        // determine triangle we are on and apply barrycentric
        float height;
